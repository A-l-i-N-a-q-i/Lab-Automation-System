...
<?php
$rejected_products = [];
$query = "SELECT * FROM product WHERE status = 'Rejected'";
...
<head>
  ...
  <title>Rejected Products - Lab Automation</title>
...
<body class="flex flex-col min-h-screen text-white relative z-0">
  ...
  <main class="flex-grow relative z-10 pt-32 pb-16 px-4 sm:px-6 lg:px-8 sm:ml-56">
    <div class="max-w-6xl mx-auto">
      <h2 class="text-3xl font-extrabold mb-10 text-center text-white drop-shadow">Rejected Products</h2>
      ...
      <div class="grid gap-6 grid-cols-1 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2">
        <?php if (!empty($rejected_products)): ?>
          ...
          <?php endforeach; ?>
        <?php else: ?>
          <p class="text-center text-gray-300 col-span-full">No rejected products found.</p>
        <?php endif; ?>
      </div>
    </div>
  </main>
